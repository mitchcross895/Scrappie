...
async def dadjoke_command(interaction: discord.Interaction):
    """Fetch a random dad joke from icanhazdadjoke.com API"""
    await interaction.response.defer()
    
    try:
        # API requires Accept: application/json header and a custom User-Agent
        headers = {
            'Accept': 'application/json',
            'User-Agent': 'Discord Bot (https://github.com/yourbot)'
        }
        
        data = await safe_api_request(Config.DAD_JOKE_API, headers=headers)
        
        if data and 'joke' in data:
            joke_text = data['joke']
            joke_id = data.get('id', 'unknown')
            
            # Create a fun embed with the dad joke
            embed = create_embed(
                "😄 Dad Joke",
                joke_text,
                discord.Color.gold()
            ...
            # Add a footer with emoji reactions
            reactions = ["😂", "🤣", "😅", "😆", "🙃", "😏"]
            footer_emoji = random.choice(reactions)
            embed.set_footer(
                text=f"{footer_emoji} Requested by {interaction.user.display_name} • ID: {joke_id}",
                icon_url=interaction.user.display_avatar.url
            )
            
            # Add a thumbnail for visual appeal
            embed.set_thumbnail(url="https://icanhazdadjoke.com/static/smile.svg")
            
            await interaction.followup.send(embed=embed)
        else:
            embed = create_embed(
                "❌ Error",
                "Couldn't fetch a dad joke right now. Try again!",
                discord.Color.red()
            )
            await interaction.followup.send(embed=embed)
        ...
        embed = create_embed(
            "❌ Error",
            "Something went wrong fetching the joke. Try again later!",
            discord.Color.red()
        )
        await interaction.followup.send(embed=embed)
